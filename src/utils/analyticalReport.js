...
export const generatePrepaidAnalyticalPDF = async (analyticsData, filterInfo = {}) => {
  ...
  const reportTitle = filterInfo.gym_name
    ? `${filterInfo.gym_name} - Prepaid Analytics Report`
    : 'Prepaid Analytics Dashboard Report';
  ...
  const kpis = [
    { label: 'Total Revenue', value: `₱${(analyticsData.prepaid_revenue || 0).toLocaleString()}`, color: [16, 185, 129] },
    { label: 'Members Inside', value: analyticsData.active_members_inside || 0, color: [99, 102, 241] },
    { label: 'Total Logins', value: analyticsData.total_logins || 0, color: [139, 92, 246] },
    { label: 'Total Transactions', value: (analyticsData.topups_vs_deductions?.topups || 0) + (analyticsData.topups_vs_deductions?.deductions || 0), color: [245, 158, 11] },
    { label: 'Peak Hour', value: analyticsData.peak_hour || '—', color: [75, 85, 99] },
  ];
  ...
  if (analyticsData.most_active_members && analyticsData.most_active_members.length > 0) {
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
    doc.setTextColor(0);
    doc.text('TOP 3 MOST ACTIVE MEMBERS', 14, yPosition);
    ...
    const topMembers = analyticsData.most_active_members.slice(0, 3);
    const memberCardWidth = 55;
    const memberCardHeight = 28;
    const medals = ['1st', '2nd', '3rd'];
    const cardColors = [
    ...
    topMembers.forEach((member, index) => {
      ...
      doc.setTextColor(0);
      doc.setFont('helvetica', 'bold');
      const name = (member.full_name || 'N/A').substring(0, 15);
      const nameWidth = doc.getTextWidth(name);
      doc.text(name, x + (memberCardWidth - nameWidth) / 2, y + 15);
      ...
      doc.setTextColor(100);
      doc.setFont('helvetica', 'normal');
      const visits = `${member.login_count || 0} visits`;
      const visitsWidth = doc.getTextWidth(visits);
      doc.text(visits, x + (memberCardWidth - visitsWidth) / 2, y + 20);

      doc.setFontSize(6);
      const rfid = (member.rfid_tag || 'N/A').substring(0, 12);
      const rfidWidth = doc.getTextWidth(rfid);
      doc.text(rfid, x + (memberCardWidth - rfidWidth) / 2, y + 24);
    });
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  doc.text('TOP-UPS VS SESSION DEDUCTIONS', 14, yPosition);
  doc.text('SWIFTPASS COMMISSION', 110, yPosition);
  yPosition += 5;

  const topupsChart = await captureChartAsImage('topupsVsDeductionsChart');
  const commissionChart = await captureChartAsImage('commissionChart');

  if (topupsChart) {
    doc.addImage(topupsChart, 'PNG', 14, yPosition, 80, 80);
  }

  if (commissionChart) {
    doc.addImage(commissionChart, 'PNG', 110, yPosition, 80, 80);
  }
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  doc.text('REVENUE BY ACTION TYPE', 14, yPosition);
  yPosition += 5;

  const revenueChart = await captureChartAsImage('revenueByActionChart');
  if (revenueChart) {
    doc.addImage(revenueChart, 'PNG', 14, yPosition, 180, 70);
    yPosition += 75;
  }
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  doc.text('LOGINS BY HOUR (24 Hours)', 14, yPosition);
  yPosition += 5;

  const scansChart = await captureChartAsImage('scansByHourChart');
  if (scansChart) {
    doc.addImage(scansChart, 'PNG', 14, yPosition, 180, 70);
    yPosition += 75;
  }

  if (analyticsData.recent_events && analyticsData.recent_events.length > 0) {
    doc.addPage();
    yPosition = 20;

    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
    doc.text(`RECENT PREPAID EVENTS (${analyticsData.recent_events.length})`, 14, yPosition);
    yPosition += 8;
    ...
    analyticsData.recent_events.slice(0, 15).forEach((event, index) => {
      const row = Math.floor(index / cardsPerRow);
      const col = index % cardsPerRow;
      const x = 14 + col * (cardWidth + cardSpacing);
      const y = yPosition + row * (cardHeight + cardSpacing);
      ...
      const isTopup = event.action === 'Tapup';
      const cardColor = isTopup ? [220, 252, 231] : [254, 243, 199];
      ...
      doc.setFontSize(6);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(isTopup ? 21 : 146, isTopup ? 128 : 64, isTopup ? 61 : 0);
      doc.text(event.action || 'N/A', x + 2, y + 4);

      doc.setFontSize(8);
      doc.setTextColor(0);
      doc.setFont('helvetica', 'bold');
      const name = (event.name || 'N/A').substring(0, 18);
      doc.text(name, x + 2, y + 9);

      doc.setFontSize(6);
      doc.setTextColor(100);
      doc.setFont('helvetica', 'normal');
      doc.text(`Amount: ₱${event.amount || 0}`, x + 2, y + 13);
      doc.text(`Balance: ₱${event.balance || 0}`, x + 2, y + 16);
    });
  }
  ...
  const timestamp = new Date().toISOString().split('T')[0];
  const filterSuffix = filterInfo.filter_type ? `_${filterInfo.filter_type}` : '';
  const filename = `prepaid_analytics_report${filterSuffix}_${timestamp}.pdf`;
  doc.save(filename);
