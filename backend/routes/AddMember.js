...
router.post("/add-member", upload.single("member_image"), async (req, res) => {
  ...
  const initialBalance = parseFloat(req.body.initial_balance) || 0;
  ...
  if (payment_method.toLowerCase() === "gcash" && !gcash_reference) {
    return res.status(400).json({ message: "GCash reference is required for GCash payments." });
  }
  ...
  try {
    const [existing] = await dbSuperAdmin
      .promise()
      .query("SELECT 1 FROM MembersAccounts WHERE rfid_tag = ? LIMIT 1", [rfid_tag]);
    ...
    await dbSuperAdmin.promise().query(insertTransactionSql, [
      ...
      payment_method.toLowerCase() === "gcash" ? reference || gcash_reference : null,
      staff_name,
      plan_name || null,
    ]);
...
router.post("/add-subscription-member", upload.single("member_image"), async (req, res) => {
  ...
  if (
    ...
    !payment || !staff_name || !payment_method || !admin_id ||
  ...
  if (payment_method.toLowerCase() === "gcash" && !gcash_reference) {
    return res.status(400).json({ message: "GCash reference is required." });
  }
  ...
  try {
    ...
    await dbSuperAdmin.promise().query(insertMemberTxnSql, [
      ...
      payment_method.toLowerCase() === "gcash" ? reference || gcash_reference : null,
      staff_name,
      plan_name || subscription_type || null,
      subscription_start,
      subscription_expiry
    ]);
