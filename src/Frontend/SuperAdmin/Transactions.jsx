...
const SuperAdminTransactions = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [txnRes, paymentRes] = await Promise.all([
          ...
          api.get("/api/superadmin-payment-options"),
        ]);
  ...
  }, [search, filterMethod, filterType, transactions, startDate, endDate]);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
        ...
        >
          ...
          {paymentOptions.map((option) => (
            <option key={option.id} value={option.payment_method.toLowerCase()}>
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((txn, index) => (
                  <tr key={txn.id} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                    ...
                    <td className="px-3 py-2 font-medium text-gray-800">{txn.admin_name || "N/A"}</td>
                    <td className="px-3 py-2">{txn.gym_name || "N/A"}</td>
