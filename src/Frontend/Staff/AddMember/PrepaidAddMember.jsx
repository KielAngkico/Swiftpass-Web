    ...
    const fetchMembershipFee = async () => {
      try {
        ...
        const membershipPlan = data.find(
          (plan) => plan.plan_name === "Membership Fee" && plan.system_type === "prepaid_entry"
        );
...
const validate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({
        full_name: "", gender: "", age: "", rfid_tag: "", phone_number: "",
  ...
  return (
    <div>
      <div className="mb-5">
        <h1 className="text-xl font-semibold text-gray-900">Add Prepaid Member</h1>
        <p className="text-xs text-gray-500 mt-0.5">Fill out the form to register a new prepaid member.</p>
      </div>
