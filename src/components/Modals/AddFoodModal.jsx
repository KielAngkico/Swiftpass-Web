...
const handleSearch = async () => {
  ...
  setIsSearching(true);
  console.log("USDA URL:", USDA_SEARCH_URL);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700 mb-1">Search Food Database (USDA)</label>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 gap-2">
            ...
            <input
              ...
              onChange={handleInputChange}
              placeholder="Reference Weight (g)"
              className="w-full p-2 border border-gray-300 rounded-md text-xs focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
            />
          </div>

          <div className="grid grid-cols-2 gap-2">
            <input type="number" name="calories" value={formData.calories} onChange={handleInputChange} placeholder="Calories" className="p-2 border border-gray-300 rounded-md text-xs focus:ring-1 focus:ring-blue-500"/>
            <input type="number" name="protein" value={formData.protein} onChange={handleInputChange} placeholder="Protein (g)" className="p-2 border border-gray-300 rounded-md text-xs focus:ring-1 focus:ring-blue-500"/>
            <input type="number" name="carbs" value={formData.carbs} onChange={handleInputChange} placeholder="Carbs (g)" className="p-2 border border-gray-300 rounded-md text-xs focus:ring-1 focus:ring-blue-500"/>
            <input type="number" name="fats" value={formData.fats} onChange={handleInputChange} placeholder="Fats (g)" className="p-2 border border-gray-300 rounded-md text-xs focus:ring-1 focus:ring-blue-500"/>
          </div>

          <div className="grid grid-cols-2 gap-2">
            <label className="flex items-center text-xs">
              <input type="checkbox" checked={formData.is_meat} onChange={(e)=>setFormData(prev=>({...prev,is_meat:e.target.checked}))} className="mr-2"/>
              Is Meat
            </label>
            <label className="flex items-center text-xs">
              <input type="checkbox" checked={formData.is_red_meat} onChange={(e)=>setFormData(prev=>({...prev,is_red_meat:e.target.checked}))} className="mr-2"/>
              Is Red Meat
            </label>
          </div>
          ...
          <div className="flex gap-2 pt-4">
            <button type="button" onClick={handleClose} className="flex-1 bg-gray-500 text-white px-4 py-2 rounded-md text-xs hover:bg-gray-600">Cancel</button>
            <button type="submit" className="flex-1 bg-green-500 text-white px-4 py-2 rounded-md text-xs hover:bg-green-600">Save Food</button>
          </div>
        </form>
      </div>
    </div>
